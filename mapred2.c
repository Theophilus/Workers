...
*/
		int i;
		pthread_t *thread = malloc(sizeof(pthread_t)*num_map);
		int *threadcount  = malloc(sizeof(int)*num_map);
		//while(!feof(f_in)){
			for (i=0;i<num_map;i++){
				//if (feof(f_in))
					//break;
				job_num=i;				
				threadcount[i] = pthread_create( &thread[i], NULL, map, NULL);//(void*) f_in););
			}
			for (i=0;i<num_map;i++)
...
*/
		//int i;
		pthread_t *thread_red = malloc(sizeof(pthread_t)*num_red);
		int *threadcount_red  = malloc(sizeof(int)*num_red);
		//while(!feof(f_in)){
			for (i=0;i<num_red;i++){
				//if (feof(f_in))
					//break;
				job_num=i;
				threadcount_red[i] = pthread_create( &thread_red[i], NULL, reduce, NULL);//(void*) f_in););
			}
			for (i=0;i<num_red;i++)
				pthread_join(thread_red[i], NULL);
		//}
		free(thread_red);
		free(threadcount_red); 
